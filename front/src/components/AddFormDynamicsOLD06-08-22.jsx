...
const AddFormDynamics = (props) => {
  ...
  // const Ultimonrodecomanda = function (numbercomanda) {
  //   if (!comanda.loading) {
  //     if (comanda.data.comandas.length != 0)
  //       numbercomanda =
  //         comanda.data.comandas[comanda.data.comandas.length - 1].nrodecomanda;
  //     else numbercomanda = 0;
  //   }

  //   return numbercomanda;
  // };

  //var x = Ultimonrodecomanda();
  ...
  const handleInputChange = (index, event) => {
    ...
    console.log("valueindex", inputFields[index].codprod);
    if (
      ...
      for (let i = 0; i < inputFields.length; i++) {
        ...
        getProducservId(inputFields[i].codprod).then((stk) => {
          ...
          let resstk =
            ...
            parseInt(inputFields[i].cantidad);
          // let resstk = 10
          // if (!producservs.loading) {
  ...
  const handleSubmit = (e) => {
    ...
    // for (var i = 0, max = inputFields.length; i < max; i += 1) {
    //   const duplicado = valorArray.includes(inputFields[i].codprod);
    //   console.log("ES DUPLICADO: ", duplicado + " " + i + " " + valorArray[i]);
    // }
    ...
    // funcion controla si hay productos repetidos en inputFields
    // evita q se carguen dos productos iguales

    // var repiteArray = [];
    // var contador;
    // for (var i = 0, max = inputFields.length; i < max; i += 1) {
    //   contador = 0;
    //   for (var j = 0, max = valorArray.length; j < max; j += 1) {
    //     if (valorArray[j] === inputFields[i].codprod) {
    //       contador++;
    //     }
    //     repiteArray[i] = contador;
    //   }
    // }
    // console.log(repiteArray);

    // for (var i = 0, max = repiteArray.length; i < max; i += 1) {
    if (found >= 2) {
      console.log("Se repite");
      // i = repiteArray.length;
      alert("Existen productos duplicados");
      e.preventDefault();
      // setGrabar(false);
      save = false;
    } else {
